...
m2 = Member.create(user: "test1", password: "test1", password_confirmation: "test1", administrator: true)
...
p[1] = Place.create(
	...
	lat:40.825202 , 
	lng:140.745126 , 
	description: "ナポリタン気さくなマスターが、昭和52年から作り続けているメニューのひとつでもある。ナポリタンは、まさに昔ながらのケチャップが効いた懐かしい味。ボリューム満点で味も満足！",
	address:"", 
	tel:"", time:"7:00~20:00 定休日:日" , 
	type_id: t1.id)
...
p[2] = Place.create(
	...
	lat:40.823541 , 
	lng:140.746462, 
	description: "喫茶店のエスプレッソコーヒーを使用したエスプレッソコーヒーゼリー。クリームはふわふわ。ガムシロップをお好みの分量で入れられるため、苦め、甘め、どちらも味わうことが出来ます。",
	address:"青森県青森市本町1-2-28 柳町ビル1F", 
	tel:"017-776-8702", 
	time:"7:30~20:00(14:00~1600は休み) 定休日:日" , 
	type_id: t1.id)
...
p[3] = Place.create(
	...
	lat:40.823744 ,  
	lng:140.745756 ,  
	description: "おいしいお肉とホルモン、お酒、活気があるお店なので、新世界に訪れたお客さんは、元気と力がつくことまちがいなし！塩ガツは焼いてからたれをつけないでレモンをしぼって食べる一品。ガツとは豚の胃の部分で、歯ごたえがとてもこりこりしている。ホルモンが苦手の人でも満足できる！",
	address:"青森県青森市本町１−２−１", 
	tel:"017-775-3729", 
	time:"17:00~24:00 定休日:月" , 
	type_id: t1.id)
...
p[4] = Place.create(
	...
	lat:40.823644 , 
	lng:140.744818 , 
	description: "青森県産の蕎麦粉と鴨を使用。冷たい蕎麦を温かいつけ汁でいただく逸品。つけ汁は具沢山。臭みのない鴨が非常に美味しい。食後に蕎麦湯としていただくのもまたオツ！地産地消を推進し、青森県産の食材を使用。蕎麦はもちろんのこと、青森名物生姜味噌おでんや、柳町の逸品冷やし五目蕎麦などもございます。蕎麦屋でお酒を愉しみ、シメに蕎麦。いかがですか。",
	address:"青森市長島1-6-5", 
	tel:"017-722-4967", 
	time:"10:30～20:30" , 
	type_id: t1.id)
...
p[5] = Place.create(
	...
	lat:40.823412 , 
	lng:140.744767 , 
	description: "旬香御膳。その日仕入れた食材を使用しているため、素材の味を生かした新鮮で美味しい魚介類をいただける。時期によりメニューが変更されるので、来店前にお問い合わせください。あんみつは程々の甘さで食後サッパリ。ほうじ茶アイスは絶品。とにかく新鮮！落ち着いた空間と旬香る料理でランチと宴会を楽しめる。",
	address:"青森市長島1-6-6 クロスタワー ア・ベイ 2F", 
	tel:"017-721-3255", 
	time:"11:00～14:30　17:00～23:00　定休日:日" , 
	type_id: t1.id)
...
p[6] = Place.create(
	...
	lat:40.825388 , 
	lng:140.745929 , 
	description: "カプリチョーザのトマトとニンニクのスパゲティは、常連も初めての人も必ず頼む人気の逸品。細麺に絡まるトマトソースは、酸味とガーリックのスパイシーさが広まり、絶品。１人でも来れる居心地の良さとボリュームたっぷりで友達同士で来るもOK！夜限定メニューでお酒やおつまみもあるので、ぜひカプリチョーザへ！",
	address:"青森県青森市本町1-1-1", 
	tel:"017-732-1331", 
	time:"11：30－23：00(LO 22：00)" , 
	type_id: t1.id)
...
p[7] = Place.create(
	...
	lat:40.8245 , 
	lng:140.746781 , 
	description: "パエリヤこの料理は多くの人が必ず注文する一品。客数に合わせて量を変えられるのでパーティーにもお勧め。見た目は彩り豊かで、魚介や鶏肉などがふんだんに使われているため、ボリュームも満点！！！本場スペインの伝統的な料理や雰囲気が堪能できるお店。お店を気に入ってリピートするお客さんも多い。青森県産の食材を使っているので安心。",
	address:"青森県青森市本町1-1-35 ", 
	tel:"017-773-1535", 
	time:"" , 
	type_id: t1.id)
...
p[8] = Place.create(
	...
	lat:40.823381 , 
	lng:140.744751 , 
	description: "TONY  ROMA’S自慢のオリジナルベイビーバックリブ。子豚の背中のお肉を使い、オリジナルBBQソースで焼き、外はパリッと中はジューシー。みんなで楽しんで食べれる！ほかにも本場風のローマバーガーや日替わりのお得なバリューランチもおすすめ☆誕生日や記念日などを言ってもらえればサプライズもしてくれる！",
	address:"青森県青森市長島1-6-6クロスタワーA－BAY２階", 
	tel:"017-776-5551", 
	time:"11：00～23：00(LO 22：00)" , 
	type_id: t1.id)
...
p[9] = Place.create(
	...
	lat:40.824616 , 
	lng:140.745868 , 
	description: "本場スイスのカーリングチームのコーチ、監督が絶賛の厳選されたチーズを使用しており、少し塩気を感じることができる。パン、エビ、ブロッコリー、トマトをチーズと絡めて食べると濃厚なチーズと絡めて食べると濃厚なチーズな中にも食材の味を感じられる逸品である。チーズフォンデュは子供向けと大人向けに作ることが可能で、メニューすべてお客様の希望に応えてくれる。お酒の種類も多く、飲み会にもオススメ。従業員全員気さくで、とてもいい雰囲気のお店。",
	address:"青森県青森市本町1-1-40", 
	tel:"017-775-2588", 
	time:"ランチ11:30～14:00 ディナー17:00～24:00(LO 23:30)" , 
	type_id: t1.id)
